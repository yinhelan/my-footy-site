// ===== components/tools/MatchInsights.tsx (a11y+SEO copy ready) =====
...
function NumberInput(
  props: React.InputHTMLAttributes<HTMLInputElement> & { id: string; label: string; hint?: string }
) {
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      />
      {hint && <span className="text-xs text-neutral-500" id={`${id}-hint`}>{hint}</span>}
    </div>
  );
}
...
export default function MatchInsights() {
  ...
  return (
    <div className="grid gap-6">
      {/* 输入区 */}
      <section className="grid gap-4 p-4 bg-white rounded-2xl shadow-sm border" aria-labelledby="mi-inputs">
        <h2 id="mi-inputs" className="text-lg font-semibold">Input</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <NumberInput id="homeTeam" label="Home team" value={home} onChange={(e)=>setHome((e.target as HTMLInputElement).value)} />
          <NumberInput id="awayTeam" label="Away team" value={away} onChange={(e)=>setAway((e.target as HTMLInputElement).value)} />
          <NumberInput id="maxGoals" label="Max goals K" type="number" step="1" min={4} max={12} value={maxGoals}
            onChange={(e)=>setMaxGoals(Number((e.target as HTMLInputElement).value)||8)} hint="矩阵维度（默认 8）" />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <NumberInput id="lambdaH" label="λ(Home)" type="number" step="0.05" value={lambdaH}
            onChange={(e)=>setLambdaH(Number((e.target as HTMLInputElement).value)||0)} hint="主队期望进球" />
          <NumberInput id="lambdaA" label="λ(Away)" type="number" step="0.05" value={lambdaA}
            onChange={(e)=>setLambdaA(Number((e.target as HTMLInputElement).value)||0)} hint="客队期望进球" />
          <NumberInput id="bankroll" label="Bankroll" type="number" step="1" value={bankroll}
            onChange={(e)=>setBankroll(Number((e.target as HTMLInputElement).value)||0)} />
          <NumberInput id="kellyCap" label="Kelly cap" type="number" step="0.01" value={cap}
            onChange={(e)=>setCap(Number((e.target as HTMLInputElement).value)||0)} hint="单注上限（比例）" />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <NumberInput id="oddsH" label="Market odds - Home" type="number" step="0.01" value={oddsH}
            onChange={(e)=>setOddsH((e.target as HTMLInputElement).value===""?"":Number((e.target as HTMLInputElement).value))} />
          <NumberInput id="oddsD" label="Market odds - Draw" type="number" step="0.01" value={oddsD}
            onChange={(e)=>setOddsD((e.target as HTMLInputElement).value===""?"":Number((e.target as HTMLInputElement).value))} />
          <NumberInput id="oddsA" label="Market odds - Away" type="number" step="0.01" value={oddsA}
            onChange={(e)=>setOddsA((e.target as HTMLInputElement).value===""?"":Number((e.target as HTMLInputElement).value))} />
        </div>
        <div className="flex flex-wrap gap-3">
          <button onClick={copySummary} className="px-4 py-2 rounded-xl bg-indigo-600 text-white hover:bg-indigo-700"
            aria-label="Copy text summary of model results and stakes">复制摘要</button>
          <button onClick={exportScoreCSV} className="px-4 py-2 rounded-xl bg-neutral-200 hover:bg-neutral-300"
            aria-label="Export top 5 scorelines as CSV">导出 Top5 CSV</button>
        </div>
      </section>

      {/* 模型输出 */}
      <section className="grid gap-4 p-4 bg-white rounded-2xl shadow-sm border" aria-labelledby="mi-model">
        <h2 id="mi-model" className="text-lg font-semibold">Model 1X2（Poisson ⟶ 归一）</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <section className="grid gap-4 p-4 bg-white rounded-2xl shadow-sm border" aria-labelledby="mi-market">
        <h2 id="mi-market" className="text-lg font-semibold">Market vs Model（含 Kelly）</h2>
        {oddsH && oddsD && oddsA ? (
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 border rounded-xl">
            ...
            <p className="text-xs text-neutral-500">说明：Kelly 使用模型概率 p 与市场赔率 o 计算，若无优势则为 0；默认单注上限为 {Math.round(cap*100)}%。</p>
          </>
        ) : (
